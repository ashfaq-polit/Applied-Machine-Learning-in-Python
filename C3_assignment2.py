...
def answer_three():
    ...
    #plt.scatter(range(0,10),r2_train,marker = '.')
    #plt.scatter(range(0,10),r2_test,marker = '*')
...
def answer_four():
    ...
    clf_lasso = Lasso(alpha=0.01, max_iter = 10000)
    clf_lasso.fit(X_train_poly, y_train.reshape(-1,1))
    y_pred_lasso = clf_lasso.predict(X_test_poly)
...
def answer_five():
    ...
    type(X_train2.columns)
    type(X_train2.columns.values)

    df = pd.DataFrame({'x':X_train2.columns.values, 'y':A})

    df_5 = df.sort_values('y',ascending=False)

    list = df_5.head(5)[['x']]
    type(list['x'])

    list_5 = list['x'].tolist()
...
def answer_seven():
    ...
    #plt.semilogx(np.logspace(-4, 1, 6),train_mean,marker = '.')
    #plt.semilogx(np.logspace(-4, 1, 6),test_mean,marker = '*')
